...
3. **Auto-bans abusive keys**:
   - **10 consecutive 429s** → key removed from rotation for **5 hours**
   - **10 more consecutive 429s** after the 5h cooldown → key **permanently banned** until next restart
   - A single **successful request** immediately clears all bans and resets the 429 counter

#### Key state file

Ban state is persisted to `.ghcp2oc_key-state.json` in the cache directory. You can manually edit this file to release bans or adjust counters:
...
| `bannedUntil` | ISO timestamp when the key returns to rotation |
| `permanentlyBanned` | `true` if key hit 20 total consecutive 429s (two strikes) |

To manually unban a key, delete its entry or set `permanentlyBanned: false` and remove `bannedUntil`, then restart the proxy.
