...
| [yxlao/deepseek-cursor-proxy](https://github.com/yxlao/deepseek-cursor-proxy) | Multi-tier reasoning key system (message signature + tool_call_id + tool_call_signature + tool_name), conversation scope hashing (SHA-256 of canonicalized prefix), tool call signature normalization for stable cross-turn caching, incremental streaming reasoning caching, `reasoning_content` recovery via history truncation (3-tier: disable thinking → strip assistant/tool history), thinking display as Cursor-visible collapsible `<details><summary>Thinking</summary>...</details>` blocks, `assistant_needs_reasoning_for_tool_context` detection, recovery boundary notices, `functions`/`function_call` → `tools`/`tool_choice` legacy conversion, `reasoning_effort` alias normalization, content prefix stripping for echoed thinking blocks |
