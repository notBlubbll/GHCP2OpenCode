...
move /y .dist\gc2oc.exe .dist\gc2oc >nul
...
echo     gc2oc                  Bun standalone server (~112 MB)
...
echo.
echo   Install:  sc create gc2oc binPath= "\"%%CD%%\.dist\service.exe\"" start= auto
echo   Start:    sc start gc2oc
echo   Stop:     sc stop gc2oc
echo   Uninstall:sc delete gc2oc
echo.
...
class StartWrapper
{
    ...
    {
        string name = Environment.GetEnvironmentVariable("GC2OC_SERVICE_NAME");
    ...
    {
        string gc2ocPath = Path.Combine(baseDir, "gc2oc");
        if (File.Exists(gc2ocPath))
        {
            if (interactive) Log("[INFO] Runtime: Bun (standalone)");
            if (interactive) Log("");
            return RunProcessTracked(gc2ocPath, "", baseDir);
        }

        if (interactive)
        {
            LogErr("[ERROR] gc2oc not found in " + baseDir);
            LogErr("       Expected: gc2oc next to service.exe");
